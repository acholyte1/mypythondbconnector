...
def save_login_info(host, port, user, password, database):
...
def launch_login_gui():
    ...
    # DBMS별 기본 포트
    def set_default_port(event=None):
        dbms = db_type_var.get().lower()
        default = DEFAULT_PORTS.get(dbms, 3306)
        # 저장된 값 없거나, 지금 입력칸이 비어있으면 기본 포트 채움
        cur = entry_port.get().strip()
        if not cur:
            entry_port.delete(0, tk.END)
            entry_port.insert(0, str(default))
    ...
    combo.pack()
    combo.bind("<<ComboboxSelected>>", set_default_port)
    ...
    if not entry_port.get().strip():
        set_default_port()  # 저장값이 없으면 기본 포트 채움
    ...
    def on_connect():
        ...
        # 연결 시도
        conn, error = get_connection(host, port, user, password, database)
        # 권한 확인
        ...
        if error:
            ...
            if remember_var.get():
                ...
                if new_info != saved_info:
                    save_login_info(host, port, user, password, database)
            else:
